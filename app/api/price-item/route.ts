...
import { isServiceAvailable, getServiceStatus } from "@/lib/circuit-breaker"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const openaiStatus = getServiceStatus(OPENAI_SERVICE)

    return NextResponse.json({
      price: priceRange,
      source: isServiceAvailable(OPENAI_SERVICE) && hasOpenAIKey() ? "ai-fallback" : "fallback",
      openaiStatus: {
